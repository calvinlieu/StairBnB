...
function CreateBookingForm({ spot, star, review }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      dispatch(createBooking(spot.id, { startDate, endDate }))
    ...
    <div className="form-container">
      ...
      >
        ...
        <div className="booking-content-wrapper">
          <span className="price-wrapper"><strong>${spot.price}</strong> night</span>
          <div className="booking-rating-wrapper">
            <i className="fa-solid fa-star"></i>
            <span> {star} · <u>{review.length} {review.length === 1 ? "review" : "reviews"}</u></span>
          </div>
        </div>
